...
exports.getHistory = async (req, res) => {
  try {
    const history = await StockMovement.getHistoryByProduct(
...
exports.getFilteredHistory = async (req, res) => {
  try {
    ...
    const data = await StockMovement.getFilteredHistory(
      product_id,
      start,
      end
    );
...
exports.getStockSummary = async (req, res) => {
  try {
    ...
    if (!product_id || !start || !end) {
      return res
        .status(400)
        .json({ message: "product_id, start and end are required" });
    }
